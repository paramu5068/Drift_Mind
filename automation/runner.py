...
def generate_automation_test_cases():
    ...
    for m_idx in range(2, 12):
        ...
        for idx, (action, exp_desc) in enumerate(feature_actions, 1):
            ...
            cases.append({
                "Test ID": f"AUT_{mod_name[:3].upper()}_{idx:03d}",
...
def generate_unit_test_cases():
    ...
    for m_idx in range(12):
        ...
        for idx in range(count):
            ...
            cases.append({
                "Test ID": f"UNT_{mod_name[:3].upper()}_{idx:03d}",
...
def generate_vulnerability_test_cases():
    ...
    for m_idx in range(12):
        ...
        for idx in range(count):
            ...
            cases.append({
                "Test ID": f"VUL_{mod_name[:3].upper()}_{idx:03d}",
...
def generate_validation_test_cases():
    ...
    for m_idx in range(12):
        ...
        for idx in range(count):
            ...
            cases.append({
                "Test ID": f"VAL_{mod_name[:3].upper()}_{idx:03d}",
...
def generate_deploy_test_cases():
    ...
    for m_idx in range(12):
        ...
        for idx in range(count):
            ...
            cases.append({
                "Test ID": f"DEP_{mod_name[:3].upper()}_{idx:03d}",
...
def generate_load_test_cases():
    ...
    for m_idx in range(12):
        ...
        for idx in range(count):
            ...
            cases.append({
                "Test ID": f"LOD_{mod_name[:3].upper()}_{idx:03d}",
...
def generate_passed_test_cases():
    ...
    for m_idx in range(12):
        ...
        for idx in range(count):
            ...
            cases.append({
                "Test ID": f"PAS_{mod_name[:3].upper()}_{idx:03d}",
...
def create_report_workbook(filename, title_text, test_records):
    ...
    # Resolve target directory paths dynamically (works both locally and in CI/CD)
    ...
    save_dirs = [
        ...
        root_dir,
        parent_dir,
        os.path.join(script_dir, "Test Results"),
        os.path.join(script_dir, "Test Results", "Excel"),
        os.path.join(root_dir, "Test Results"),
...
"""
    ...
    summary_dirs = [
        os.path.join(script_dir, "Test Results", "Summary"),
        os.path.join(root_dir, "Test Results", "Summary")
    ]
    for s_dir in summary_dirs:
        os.makedirs(s_dir, exist_ok=True)
        with open(os.path.join(s_dir, "summary.md"), "w", encoding="utf-8") as f:
